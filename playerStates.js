...
export class Running extends State {
    ...
    handleInput(input) {
        this.game.particles.unshift(new Dust(this.game, this.game.player.x + this.game.player.width * .6, this.game.player.y + this.game.player.height));
...
export class Diving extends State {
    ...
    handleInput(input) {
        ...
        if (this.game.player.onGround()) {
            this.game.player.setState(states.RUNNING, 1);
            for (let i = 0; i < 30; i++) {
                this.game.particles.unshift(new Splash(this.game, this.game.player.x + this.game.player.width * .5, this.game.player.y + this.game.player.height * .5));
            }
        } else if (input.includes('Enter') && this.game.player.onGround()) {
